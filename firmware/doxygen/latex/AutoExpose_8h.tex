...
\begin{DoxyCompactList}\small\item\em {\ttfamily max\+\_\+exposure} is 1.\+31 seconds (65535 20µs-\/cycles)~\newline
 This is the 16-\/bit limit, U\+I\+N\+T16\+\_\+\+M\+AX, but it is also a practical limit on waiting time for one frame. \end{DoxyCompactList}\end{DoxyCompactItemize}


\doxysubsection{Detailed Description}
\doxysubsection*{Global Auto\+Expose config}

\doxysubsubsection*{Auto\+Expose maximum number of tries }

Auto\+Expose gives up if it cannot hit the target peak counts range after {\ttfamily max\+\_\+tries} attempts. Recommended value is {\bfseries{10}}. Use smaller numbers to speed up Auto\+Expose. Set the value to {\bfseries{1}} to manually trigger each iteration and get a sense of how many tries are necessary to hit the target range. The range of allowed values is 1 to 255.

\doxysubsubsection*{Auto\+Expose pixel range }
...
With binning on, the default pixel range is also the allowed range of values. Attempting to exceed this range, it is clamped at these boundaries. With binning off, the allowed range is {\ttfamily start\+\_\+pixel = 14}, {\ttfamily stop\+\_\+pixel = 784}.

\doxysubsubsection*{Auto\+Expose target peak counts range }
...
{\ttfamily max\+\_\+exposure} is 1.\+31 seconds (65535 20µs-\/cycles)~\newline
 This is the 16-\/bit limit, U\+I\+N\+T16\+\_\+\+M\+AX, but it is also a practical limit on waiting time for one frame. 
...
\item update the Python A\+PI by changing all occurrences of {\ttfamily cycles} in the J\+S\+ON file from size\+:2 to size\+:4.
\end{DoxyItemize}