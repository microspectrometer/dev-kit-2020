Mock functions for C unit tests.\hypertarget{md_mock_c_README_autotoc_md45}{}\doxysection{What are unit tests and what is a mock?}\label{md_mock_c_README_autotoc_md45}
A unit test checks that a particular function does a particular thing under a particular condition. Mocks enable unit tests to say "{}here is a list of expected calls and the arguments they should be called with under this scenario, tell me if my code is producing the same list"{}.\hypertarget{md_mock_c_README_autotoc_md46}{}\doxysubsection{Table of Contents}\label{md_mock_c_README_autotoc_md46}
...
\end{DoxyItemize}\hypertarget{md_mock_c_README_autotoc_md47}{}\doxysection{mock-\/c repository contents}\label{md_mock_c_README_autotoc_md47}
...
\end{DoxyCode}
\hypertarget{md_mock_c_README_autotoc_md48}{}\doxysubsection{mock-\/c tests folder}\label{md_mock_c_README_autotoc_md48}
{\ttfamily tests/} contains the unit tests for testing {\bfseries{mock-\/c}}. Ignore {\ttfamily tests/} unless you are changing {\ttfamily mock-\/c}.\hypertarget{md_mock_c_README_autotoc_md49}{}\doxysubsection{mock-\/c code folder}\label{md_mock_c_README_autotoc_md49}
...
\end{DoxyItemize}
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{md_mock_c_README_autotoc_md50}{}\doxysubsection{mock-\/c build folder}\label{md_mock_c_README_autotoc_md50}
{\ttfamily build/} contains the {\bfseries{mock-\/c}} object files that unit tests are linked with. The object files are not in this repository; your project compiles them. And your project links with these object files when it builds its test runner application.\hypertarget{md_mock_c_README_autotoc_md51}{}\doxysection{Integrating mock-\/c in your project}\label{md_mock_c_README_autotoc_md51}
...
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{md_mock_c_README_autotoc_md52}{}\doxysubsection{Compiling mock-\/c}\label{md_mock_c_README_autotoc_md52}
...
\end{DoxyCode}
\hypertarget{md_mock_c_README_autotoc_md54}{}\doxysubsection{Example recipe for a unit test application}\label{md_mock_c_README_autotoc_md54}
...
\end{DoxyCode}
\hypertarget{md_mock_c_README_autotoc_md55}{}\doxysection{Example usage in a test}\label{md_mock_c_README_autotoc_md55}
TODO(sustainablelab)\+: Document how to use this thing\hypertarget{md_mock_c_README_autotoc_md56}{}\doxysection{About mock-\/c}\label{md_mock_c_README_autotoc_md56}
{\bfseries{mock-\/c}} is entirely in C, no C++. I\textquotesingle{}d like to make it dependency-\/free one day, but for now it requires {\bfseries{GLib}}.\hypertarget{md_mock_c_README_autotoc_md57}{}\doxysubsection{mock-\/c is built on top of these libraries}\label{md_mock_c_README_autotoc_md57}
...
\end{DoxyItemize}
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{md_mock_c_README_autotoc_md58}{}\doxysubsection{Unity Test}\label{md_mock_c_README_autotoc_md58}
...
The original \mbox{\hyperlink{md_mock_c_LICENSE_Unity_Test}{$\ast$$\ast$\+Unity Test$\ast$$\ast$ license}} is included in this repository, as required by the standard MIT license.\hypertarget{md_mock_c_README_autotoc_md59}{}\doxysubsection{GLib}\label{md_mock_c_README_autotoc_md59}
...
\end{DoxyCode}
\hypertarget{md_mock_c_README_autotoc_md60}{}\doxysubsection{Building up the Makefile for unit tests}\label{md_mock_c_README_autotoc_md60}
...
\end{DoxyCode}
\hypertarget{md_mock_c_README_autotoc_md61}{}\doxysection{More explanation about what this is}\label{md_mock_c_README_autotoc_md61}
To \char`\"{}mock\char`\"{} a function is to stub it with a {\bfseries{fake}} version that records when (in the call order) it was called and what arguments it was called with.\hypertarget{md_mock_c_README_autotoc_md62}{}\doxysubsection{TDD terminology}\label{md_mock_c_README_autotoc_md62}
...
TLDR\+: Mocking is for the {\itshape small} subset of unit tests where the goal is to check whether the function under test is calling other functions and calling them with the correct arguments.\hypertarget{md_mock_c_README_autotoc_md63}{}\doxysubsection{Reinventing wheels}\label{md_mock_c_README_autotoc_md63}
...
When the function under test calls fake functions, the fake function writes its name and its arguments to the global list. The test is to compare the expected list of calls against what was recorded in the global list and identify where something unexpected happened.\hypertarget{md_mock_c_README_autotoc_md64}{}\doxysubsection{Avoid mocking as much as possible}\label{md_mock_c_README_autotoc_md64}
