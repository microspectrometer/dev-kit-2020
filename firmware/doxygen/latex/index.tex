...
\hypertarget{index_autotoc_md0}{}\doxysection{How to use the firmware documentation}\label{index_autotoc_md0}
...
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{index_autotoc_md1}{}\doxysubsection{Firmware organization}\label{index_autotoc_md1}
...
Each folder has {\ttfamily src} (source code) and {\ttfamily test} (unit tests).\hypertarget{index_autotoc_md2}{}\doxysubsection{Library functions break with C convention}\label{index_autotoc_md2}
...
This is for \href{index.html\#autotoc_md10}{\texttt{ performance reasons}}.\hypertarget{index_autotoc_md3}{}\doxysection{Related dev-\/kit documentation}\label{index_autotoc_md3}
...
\href{https://github.com/microspectrometer/dev-kit-2020}{\texttt{ https\+://github.\+com/microspectrometer/dev-\/kit-\/2020}}\hypertarget{index_autotoc_md4}{}\doxysection{Chromation dev-\/kit hardware}\label{index_autotoc_md4}
...
There is no firmware for the spectrometer chip itself, but the chip does have programmable registers.\hypertarget{index_autotoc_md5}{}\doxysubsection{Spectrometer chip programmable registers}\label{index_autotoc_md5}
...
\end{DoxyItemize}\hypertarget{index_autotoc_md6}{}\doxysection{Serial communication and the Python API}\label{index_autotoc_md6}
\hypertarget{index_autotoc_md7}{}\doxysubsection{Protocol is in a JSON file}\label{index_autotoc_md7}
...
\end{DoxyItemize}\hypertarget{index_autotoc_md8}{}\doxysubsection{Develop and test}\label{index_autotoc_md8}
...
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{index_autotoc_md9}{}\doxysubsection{Protocol example}\label{index_autotoc_md9}
TODO\+: Add an example with the {\ttfamily usb-\/bridge} and {\ttfamily vis-\/spi-\/out} firmware that corresponds to one of the commands in the JSON file, and how the resulting call-\/and-\/response looks from Python.\hypertarget{index_autotoc_md10}{}\doxysection{Why so much inline}\label{index_autotoc_md10}
...
The simplest example is the bit manipulation functions discussed below. The same reasoning applies to all the {\ttfamily lib} code.

{\itshape This also explains why the code is full of comments like this\+:}
...
{\itshape In more complicated cases, there are several lines of assembly. Pasting the disassembly into the code helped me check that the compiler was still using the right instructions after a code change.}\hypertarget{index_autotoc_md11}{}\doxysubsection{Example of conventional C yielding inefficient assembly}\label{index_autotoc_md11}
...
Or the bit manipulation function might be called on an I/O register in the range {\ttfamily 0x00 -\/ 0x3F}. In this range the bit manipulation has to be done with entire bytes using instructions {\ttfamily in} and {\ttfamily out}.

Or the bit manipulation might be performed on dataspace, {\ttfamily 0x20 -\/ 0x5F}, or extended I/O space, {\ttfamily 0x60 -\/ 0x\+FF} in SRAM, in which case instructions like {\ttfamily ld} and {\ttfamily st} are used.

Again, the compiler does not know what register address the function is called with, so it has to use the generic {\ttfamily ld} and {\ttfamily st} instructions. The result is that calls to the bit manipulation library take a massive speed penalty.\hypertarget{index_autotoc_md12}{}\doxysubsection{Example of inline for efficient assembly}\label{index_autotoc_md12}
To avoid this, the {\ttfamily inline} copies the bit manipulation source code into all the places it is called. Now the code is part of the compilation unit that has the register addresses and the compiler knows if it can use the faster instructions.
...
The result is a glaring break with C convention. Most (all?) of the library functions are defined in {\ttfamily .h} files, with the function signatures declared in the {\ttfamily .c} file.\hypertarget{index_autotoc_md13}{}\doxysubsection{Macro USE\+\_\+\+FAKES}\label{index_autotoc_md13}
The only practical drawback to this approach is that the code gets marked up with test macros to handle function faking\+:
