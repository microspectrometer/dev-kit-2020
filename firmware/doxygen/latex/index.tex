...
All the useful firmware information is under \href{files.html}{\texttt{ Files}}.\hypertarget{index_autotoc_md1}{}\doxysection{Related dev-\/kit documentation}\label{index_autotoc_md1}
...
\href{https://github.com/microspectrometer/dev-kit-2020}{\texttt{ https\+://github.\+com/microspectrometer/dev-\/kit-\/2020}}\hypertarget{index_autotoc_md2}{}\doxysection{Chromation dev-\/kit hardware}\label{index_autotoc_md2}
...
There is no firmware for the spectrometer chip itself, but the chip does have programmable registers.\hypertarget{index_autotoc_md3}{}\doxysubsection{Spectrometer chip programmable registers}\label{index_autotoc_md3}
...
\end{DoxyItemize}\hypertarget{index_autotoc_md4}{}\doxysection{Serial communication and the Python API}\label{index_autotoc_md4}
\hypertarget{index_autotoc_md5}{}\doxysubsection{Protocol is in a JSON file}\label{index_autotoc_md5}
...
\end{DoxyItemize}\hypertarget{index_autotoc_md6}{}\doxysubsection{Develop and test}\label{index_autotoc_md6}
...
To quickly check that the firmware and Python API are in complete agreement about the serial communication protocol, run \href{https://github.com/microspectrometer/dev-kit-2020/blob/master/python/microspeclib/system-tests.py}{\texttt{ system-\/tests.\+py}}. This script runs all the serial commands and checks for the expected responses from the dev-\/kit. The script is organized into classes, one class per serial command. The methods in each class are the different scenarios for testing the serial command (the different arguments the command might be called with).\hypertarget{index_autotoc_md7}{}\doxysubsection{Protocol example}\label{index_autotoc_md7}
