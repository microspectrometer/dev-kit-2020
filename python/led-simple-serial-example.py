...
def pixel_reading(usb):
    """Return next two bytes in serial buffer as a 16-bit word."""
    return int.from_bytes(usb.read(2), byteorder='big', signed=False)
def read_frame(usb, npixels=392):
    """Return nbytes in serial buffer (=one frame) as a numpy array of 16-bit words."""
    while (usb.inWaiting() < 2*npixels): pass
...
def test_CaptureFrame():
    ...
    _tx_and_log_cmd(commands.CaptureFrame, "Command is CaptureFrame")
    # read BRIDGE status byte, stop if it is an error
    expected = codes.OK
    reply = _rx_and_log_reply(
        device_name="BRIDGE",
    ...
    reply = _rx_and_log_reply(
        device_name="SENSOR",
        reply_type="msb of npixels",
        expected_reply_byte=expected,
        optional_expectation=f"Expect {expected}"
        )
    if reply != expected:
        _print_and_log(f"test_CaptureFrame: FAIL: Expected {expected}, received {reply}.")
        return
    expected = _lsb(npixels)
    reply = _rx_and_log_reply(
        device_name="SENSOR",
        reply_type="lsb of npixels",
        expected_reply_byte=expected,
        optional_expectation=f"Expect {expected}"
        )
    if reply != expected:
    ...
    _print_and_log(f"Rx.. SENSOR: "
        f"{len(frame)} pixels in frame (Expect {npixels} pixels)"
        )
    _print_and_log("test_CaptureFrame: PASS")
