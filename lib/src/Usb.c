...
/* =====[ Next few definitions are helpers for next CmdFn: CmdCfgLis ]===== */
...
bool CfgTurnsRowPartiallyOn(uint8_t const *cfg_bytes, uint8_t row_number)
{
    ...
    /* return !XOR( */
    /*     AllPixelsAreOff(cfg_bytes, row_mask), // false if cfg is row 1 all on */
    /*     AllPixelsAreOn(cfg_bytes, row_mask) // true if cfg is row 1 all on */
    /*     ); */
